...
def get_transform(is_train, args):
    if is_train:
        return presets.SegmentationPresetTrain(imgsz=args.imgsz, crop_size=480, backend=args.backend, use_v2=args.use_v2)
        ...
        return presets.SegmentationPresetEval(imgsz=args.imgsz, backend=args.backend, use_v2=args.use_v2)
...
def evaluate(model, data_loader, device, num_classes):
    ...
    with torch.inference_mode():
        ...
        Astrago.get_elapsed_val_time(during_time)
...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, device, epoch, print_freq, scaler=None):
    ...
    Astrago.get_elapsed_train_time(during_time)    
...
def main(args):
    ...
    for epoch in Astrago(range(args.start_epoch, args.epochs)):
        ...
        utils.save_on_master(checkpoint, os.path.join(args.output_dir, f"model_epoch.pth"))
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--output-dir", default="./train/run", type=str, help="path to save outputs")
