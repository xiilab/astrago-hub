...
class Astrago(tqdm):
    ...
    # UNet 데이터 수량 check    
    def get_data_info(train_num, val_num):
        print(f'\n[Train DATA]')
...
class Inference(tqdm):
    ...
    def format_meter(n, total, elapsed, rate=None, initial=0, *args, **kwargs):
        torch.cuda.synchronize()
        gpu_usage = sum(Inference.gpu_memory_usage) / (len(Inference.gpu_memory_usage)-1) if n != 0 else sum(Inference.gpu_memory_usage) / len(Inference.gpu_memory_usage)
        cpu_usage = sum(Inference.cpu_usage) / (len(Inference.cpu_usage)-1) if n != 0 else sum(Inference.cpu_usage) / len(Inference.cpu_usage)
